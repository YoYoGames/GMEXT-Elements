...
> [!IMPORTANT]
> When a body is a struct generated by these constructors, the wrapper calls `<Type>_validate()` before sending to ensure types are correct. Note, however, that you *can* use a struct that is not an instance of the given constructor; the validation functions only check if the right variables are present in the struct and if their value is of the expected type.
